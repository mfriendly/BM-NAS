...
class ReshapeInputLayer(nn.Module):
    def __init__(self, C_in, C, L, args):
        super().__init__()
        self.C = C
        self.L = L
        self.conv = nn.Conv1d(C_in, self.C, 1, 1)
        self.bn = nn.BatchNorm1d(self.C)

        pool_size = int(math.sqrt(self.L * 1.0))
        assert pool_size * pool_size == self.L
    ...
    def forward(self, x):
        print("first in shape", x.shape)
        out = x.unsqueeze(-1)
        #print("out = x.unsqueeze(-1)",out.shape)#
        out = out.unsqueeze(-1)
        #print(" out = out.unsqueeze(-1)",out.shape)#
        out = out.view(out.size(0), out.size(1), out.size(2), -1)
        #print(" out = out.view(out.size(0), out.size(1), out.size(2), -1)",out.shape)#

        # pool the spatial
        out = self.pool(out) #nn.AdaptiveMaxPool2d((pool_size, pool_size))
        #print(" out = self.pool(out)",out.shape)
        out = out.view(16,  29, -1) #16,32->2932
        #print("out = out.view(out.size(0), out.size(1), -1)", out.shape)

        # interpolate the temporal
       # out = self.conv(out) # self.conv = nn.Conv1d(C_in, self.C, 1, 1)
       # print("out = self.conv(out)",out.shape)## weight of size [192, 64, 1], expected input[29, 1, 16] to have 64 channels, but got 1 channels instead

       # out = self.bn(out)
       # #print(out.shape)
        out = F.relu(out) # out = self.mish(out)
        #print("after relu",out.shape)

        out = self.dropout(out)

        out = self.pool2(out)##############추가 from untimeError: Given normalized_shape=[29, 16], expected input with shape [*, 29, 16], but got input of size[16, 29, 32]
        print("final ",out.shape)
        ##final torch.Size([8, 512, 16])
           # final torch.Size([8, 512, 16])
           # final torch.Size([8, 512, 16])
           # final torch.Size([8, 512, 16]) CNN???
           # final torch.Size([29, 1, 16]) RNN
           # final torch.Size([29, 1, 16])?????  RNN
        return out
...
class Cell(nn.Module):
    ...
    def __init__(self, operation_target, configuration_indexes, connections, args):
    ...
    def _create_blocks(self):
        ...
        for b_i, block_conf in enumerate(self._configuration):
            op1_type = self._operation_target[block_conf[0]]
            op2_type = self._operation_target[block_conf[1]]
