    ...
    def __init__(self, steps, multiplier, args):
        ...
        for i in range(self._steps):
            ...
            for j in range(self.num_input_nodes+i):
                ...
                self._ops.append(op)  #operation append

        self._initialize_step_nodes(args)  #step node?
...
class FusionNetwork(nn.Module):  #####serachnetwork.py 에서불리는 class
     # 해당 클래스에서 Fusion Cell 을 불러와 Genotype 반환
    def __init__(self, steps, multiplier, num_input_nodes, num_keep_edges, args,
        ...
        self.cell = FusionCell(steps, multiplier, args)  ##FusionCell 불러와
    ...
    def forward(self, input_features):
        print('self._num_input_nodes == len(input_features)', self._num_input_nodes, len(input_features))  ### 각각 11, 9 --> 9,11

        assert self._num_input_nodes == len(input_features) ###Assertion error
    ...
    def _initialize_alphas(self): ###셀 과 셀 간의 연결 강도
    ...
    def genotype(self):
        def _parse(weights):
            ...
            for i in range(self._steps):
                ...
                for j_index, j in enumerate(from_list):
                    for k in from_list[j_index+1:]:
                        ...
                        if (j not in selected_nodes) or (k not in selected_nodes):
                            ...
                            W_j_max = max(W[j][t] for t in range(len(W[j])) if t != PRIMITIVES.index('none')) #leave stronest nodes?
                ...
                selected_nodes = list(set(selected_nodes)) ###강한 연결 선택된것 리스트
