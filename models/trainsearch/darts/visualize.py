...
def plot(genotype, filename, args, task = None): ###task??????
    ...
    with g.subgraph() as nothing:
        ...
        with c.subgraph(name='video_features', node_attr={'shape': 'box'}) as ca:
            ...
            for i in range(len(input_nodes_A)-1):
                c.edge(input_nodes_A[i], input_nodes_A[i+1], target=None)



        with c.subgraph(name='skeleton_features', node_attr={'shape': 'box'}) as cb:
            ...
            for i in range(len(input_nodes_B)-1):
                c.edge(input_nodes_B[i], input_nodes_B[i+1], target=None)
    ...
    for i in range(steps):
        ...
        with g.subgraph(name=step_node_name, node_attr={'shape': 'box'}) as c:
            ...
            for j in range(node_steps):
                ...
                # c.edge(inner_node_names[x], inner_node_names[2+j], target=x_op)
                # c.edge(inner_node_names[y], inner_node_names[2+j], target=y_op)
                c.edge(inner_node_names[x], inner_node_names[2+j], target=None)
                c.edge(inner_node_names[y], inner_node_names[2+j], target=None)

                # c.edge(inner_node_names[2+j], inner_node_names[x], target=None)
                # c.edge(inner_node_names[2+j], inner_node_names[y], target=None)

            for j in range(args.node_multiplier):
                # c.edge(inner_node_names[-(j+1)], node_z_name, target='skip')
                c.edge(inner_node_names[-(j+1)], node_z_name, target=None)
            # skip connection
            # c.edge(node_x_name, node_z_name, target=None)

        edge_x_op = genotype.edges[2*i][0]
        edge_x_from = node_names[genotype.edges[2*i][1]]
        edge_x_to = node_x_name
        # g.edge(edge_x_from, edge_x_to, target=edge_x_op)
        g.edge(edge_x_from, edge_x_to, target=None, color="blue")

        edge_y_op = genotype.edges[2*i+1][0]
        edge_y_from = node_names[genotype.edges[2*i+1][1]]
        edge_y_to = node_y_name
        # g.edge(edge_y_from, edge_y_to, target=edge_y_op)
        g.edge(edge_y_from, edge_y_to, target=None, color="blue")
...
def plot(genotype, filename, args, task=None):
    ...
    for i in range(steps):
        ...
        edge_x_to = node_x_name
        # g.edge(edge_x_from, edge_x_to, target=edge_x_op)
        g.edge(edge_x_from, edge_x_to, target=None)

        edge_y_op = genotype.edges[2*i+1][0]
        edge_y_from = node_names[genotype.edges[2*i+1][1]]
        edge_y_to = node_y_name
        # g.edge(edge_y_from, edge_y_to, target=edge_y_op)
        g.edge(edge_y_from, edge_y_to, target=None)
