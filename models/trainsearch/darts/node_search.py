...
from .fusionnet import FusionMixedOp

class NodeCell(nn.Module):  ###node step은 셀안에 동그라미 순서
    def __init__(self, node_steps, node_multiplier, args):
        ...
        self.edge_ops = nn.ModuleList() ###??? 밑에 55줄에서 사용
        ...
        for i in range(self.node_steps):  #셀 i ~ 셀 j
            for j in range(self.num_input_nodes+i):
                edge_op = FusionMixedOp(self.C, self.L, self.args)  ###operations.py에 있음  연결 강도랑 가중해서 weight...합산해서 합해줌
                # 합한 강도?... return OUT
...
class FusionNode(nn.Module): ####
    ...
    def __init__(self, node_steps, node_multiplier, args):
        ...
        self.node_cell = NodeCell(node_steps, node_multiplier, args) ## 바로 위에 클래스
        #node step은 셀안에 노드 0,1,2~~동그라미 안 순서


        self.num_input_nodes = 2    ## 인풋 노드 2개로 설정
        self.num_keep_edges = 2     ## 두개 edge keep해라
    ...
    def _initialize_betas(self):  ####베타는 Cell 안의 노드들 연결하는  op 강도 / weight
    ...
    def _initialize_gammas(self): ###셀 안의 노드 웨이트
...
if __name__ == '__main__':
    ...
    args = Args(16, 8)  ###################C=16, L=8 채널수랑 fusioncell수? 바로 위에 Args... 뭘반환??????????ㄴ
    node_cell = NodeCell(2, 1, args)  #########맨위에 클래스  Node Cell은 out을 반환
    fusion_node = FusionNode(2, 1, args) ########두번째 나오는 클래스, NodeCell을 인풋으로 받음
    #FusNode는 Fusion GENE 반환
