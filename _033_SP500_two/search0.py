...
chart_dir ='/nas3/mink/NAS/feature_fusion/charts/' +stock_index+ '_' +args.draw_fnc+'/'  +args.coltype # /미포함
...
test_dataset = StockChartDataset_candle(chart_dir + '/test/candlestick/',
                                      transforms = transform)
####################################################################
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size = args.batch_size, shuffle = True, num_workers = 0,drop_last=False) ######배치사이즈 1->64->1 2.12
valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size = args.batch_size,  shuffle = True,  num_workers = 0,drop_last=False)
...
##################
    ...
    def validate(self, val_loader, criterion):
        ...
        with torch.no_grad():
            with tqdm(valid_loader) as progress:
                for epoch, (x, target) in enumerate(progress):
    ...
    def test(self, test_loader, criterion):###############잘못됬음
...
for epoch in tqdm(range(args.search_epoch_size), desc='Total Progress'):
        ...
        train_obj = trainer.train(train_loader, valid_loader,criterion, optimizer) ## Search Trainer 클래스의 train 함수
...
for ep in range(1): #
    ...
    #log1.info(f'>>>Time elapsed : {str(timedelta(seconds=elapsed))}')
    #log2.info(f'>>>Time elapsed : {str(timedelta(seconds=elapsed))}')
